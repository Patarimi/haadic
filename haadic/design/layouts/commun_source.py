...
from haadic.design.layouts.active import line, mosfet, pattern_connect
...
def layout(
    ...
    mos = mosfet(cell, width=width, length=length, nf=n_finger)
    line(cell, "gate", 0)
    line(cell, "drain", 1)
    line(cell, "gnd", 1, below=True)
    pattern_connect(cell, mos.name, ("gnd", "gate", "drain", "gate"))
